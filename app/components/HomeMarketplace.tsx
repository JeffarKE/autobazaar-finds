  ...
  return (
    <>
      ...
      <section id="listings" className="mx-auto max-w-[1360px] scroll-mt-24 px-4 py-8 sm:px-6 sm:py-14">
        ...
        {filteredCars.length ? <div className="mt-7 grid gap-x-5 gap-y-8 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">{filteredCars.map((car, index) => <VehicleCard key={car.id} vehicle={car} eager={index < 4} />)}</div> : <div className="mt-7 rounded-3xl border border-dashed p-14 text-center"><p className="text-xl font-black">No matching cars</p><button onClick={clearFilters} className="mt-4 font-bold text-emerald-600">Clear filters</button></div>}
      </section>
    </>
  );
}
